...
def test_battery_respects_power_limits():
    ...
    prices = np.array([50, 100, 150, 200, 50, 100, 150, 200] * 12)  # 96 steps
    max_power = 1.0
...
def test_battery_never_negative_power():
    ...
    charge, discharge, profit = optimize_battery(prices, capacity_mwh=3.9, max_power_mw=1.0)
...
def test_zero_volatility_gives_zero_profit():
    ...
    prices = np.full(96, 100.0)  # flat price all day
    charge, discharge, profit = optimize_battery(prices, capacity_mwh=3.9, max_power_mw=1.0)
    assert profit < 1.0  # allow tiny numerical noise, but essentially zero
...
def test_higher_volatility_gives_more_profit():
    ...
    _, _, profit_low = optimize_battery(low_volatility, capacity_mwh=3.9, max_power_mw=1.0)
    _, _, profit_high = optimize_battery(high_volatility, capacity_mwh=3.9, max_power_mw=1.0)
